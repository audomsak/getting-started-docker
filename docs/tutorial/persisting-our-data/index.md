...
1. Start a `ubuntu` container that will create a file named `/data.txt` with a random number
   between 1 and 10000.
    ...
    ```bash
    docker run -d --name ubuntu1 ubuntu bash -c "shuf -i 1-10000 -n 1 -o /data.txt && tail -f /dev/null"
    ```
...
1. Validate we can see the output by `exec`'ing into the container. To do so, can use the `docker exec` command to do the same. You need to get the
   container's ID (use `docker ps` to get it) and get the content with the following command.

    ```bash
    docker exec ubuntu1 cat /data.txt
    ```

    You should see a random number!

2. Now, let's start another `ubuntu` container (the same image) and we'll see we don't have the same
   file.

    ```bash
    docker run -it --name ubuntu2 ubuntu ls /
    ```

    And look! There's no `data.txt` file there! That's because it was written to the scratch space for
    only the first container.

3. Go ahead and remove the first container using the `docker rm -f <container-id>` command.
    ```bash
    docker rm -f ubuntu1
    ```
...
1. Once the container starts up, open the app and add a few items to your todo list.
    ...
    ![Items added to todo list](items-added.png)

2. Remove the container for the todo app. Use the Dashboard or `docker ps` to get the ID and then `docker rm -f <container-id>` to remove it.

3. Start a new container using the same command from above.
